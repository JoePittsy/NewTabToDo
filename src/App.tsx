...
function App() {
  ...
  useEffect(() => {
    ...
    if (openedProjects.length === 0 && projects.length > 0) {
      ...
      const withUncompleted = projects.filter((p) =>
      ...
      const sorted = [...withUncompleted].sort((a, b) => {
        const aUncompleted =
          a.todos?.filter((t: any) => !t.completed).length ?? 0;
        const bUncompleted =
          b.todos?.filter((t: any) => !t.completed).length ?? 0;
        if (bUncompleted !== aUncompleted) return bUncompleted - aUncompleted;
        // If tie, sort by most recent to-do createdOn
        const aLatest = Math.max(
          ...(a.todos?.map((t: any) => t.createdOn || 0) ?? [0])
        );
        const bLatest = Math.max(
          ...(b.todos?.map((t: any) => t.createdOn || 0) ?? [0])
        );
...
const RoutedApp: React.FC = () => (
  <Router>
    <Routes>
      ...
      <Route path="/dropbox_auth" element={<DropboxAuthRoute />} />
    </Routes>
  </Router>
);
...
// Dropbox authentication redirect route

import { BrowserRouter as Router, Routes, Route } from "react-router-dom";
import DropboxAuthRoute from "./cloud/DropboxAuthRoute";
