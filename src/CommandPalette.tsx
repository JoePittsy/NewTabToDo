...
export default function CommandPalette({
  ...
  const { projects, openedProjects, openProject, setOpenedProjects } =
    useProjects();
      ...
      return {
        ...
        lastNote:
          uncompleted.length > 0
            ? Math.max(...uncompleted.map((t) => t.createdOn || 0))
            : 0,
      };
    })
  ...
  const quickActions = [
    ...
    {
      name: "Open projects with active to-dos",
      icon: CheckCircleIcon,
      action: "openActiveTodos",
    },
  ...
  function handleQuickAction(action: string) {
    if (action === "openAll") {
      ...
      const activeTodoProjects = projects
        .filter(
          (p) =>
            (p.todos ?? []).some((t: any) => !t.completed) &&
            !openedProjects.includes(p.name)
        )
        .map((p) => p.name);
      if (activeTodoProjects.length > 0) {
        setTimeout(() => {
          const all = [
            ...openedProjects,
            ...activeTodoProjects.filter((n) => !openedProjects.includes(n)),
          ];
          setOpenedProjects(Array.from(new Set(all)));
        }, 0);
      }
          ...
          <Combobox
            onChange={(item: any) => {
              if (
                item &&
                item.name &&
                projects.some((p) => p.name === item.name)
              ) {
              ...
              <ComboboxInput
                ...
                onKeyDown={(e) => {
                  ...
                  if (
                    ...
                    if (listRef.current) {
                      const focused =
                        listRef.current.querySelector("[data-focus]");
                      if (focused) {
                        const options = Array.from(
                          listRef.current.querySelectorAll('[role="option"]')
                        );
                  ...
                  if (e.key === "Enter" && selectedProjects.length > 0) {
                    ...
                    setTimeout(() => {
                      setOpenedProjects(
                        Array.from(
                          new Set([...openedProjects, ...selectedProjects])
                        )
                      );
                    }, 0);
                  }
                        ...
                        {unopened.map((project, idx) => (
                          ...
                          >
                            ...
                            >
                              {project.lastNote > 0
                                ? `${
                                    project.todos.filter((t) => !t.completed)
                                      .length
                                  } - 
                                                                ${new Date(
                                                                  project.lastNote
